<?php
    ...
    switch ($oper) {
        case "Insere":
            echo "insert into pessoa (nome,cpf) values('$nome','$cpf')";
            mysqli_query($db, "insert into pessoa (nome,cpf) values('$nome','$cpf')");
            echo "Elemento inserido";
            break;
        case "Excluir":
            mysqli_query($db, "delete from pessoa where codigo= $sel");
            echo "Elemento exclu&iacutedo";
            break;
        case "Altera":
            mysqli_query($db, "update pessoa set nome='$nome',cpf='$cpf' where codigo=$codigo");
            echo "Elemento alterado";
            break;
        case "Alterar":
            if ($sel) {
                $lista = mysqli_query($db, "Select * from pessoa where codigo=$sel");
    ...
    $tabela = "<table border=1><tr><td></td><td>C&oacutedigo</td><td>Nome</td><td>CPF</td></tr>";
    $lista = mysqli_query($db, "Select * from pessoa order by codigo");
    $linhas = $lista ? mysqli_num_rows($lista) : 0;
    for ($i = 0; $i < $linhas; $i++) {
        $tabela .= "<tr><td><input type='radio' name='sel' value='" . mysql_result($lista, $i, "codigo") . "'></td>";
        $tabela .= "<td>" . mysql_result($lista, $i, "codigo") . "</td>";
        $tabela .= "<td>" . mysql_result($lista, $i, "nome") . "</td>";
        $tabela .= "<td>" . mysql_result($lista, $i, "cpf") . "</td></tr>";
    }
    $tabela .= "</table>";
    mysqli_close($db);
?>
...
<body>
    <form action="comum.php" method="post">
        ...
        if ($oper == "Novo" || $oper == "Alterar") {
            ...
            echo "C&oacutedigo<input type='text' name='codigo' value='$codigo' readonly><br>";
